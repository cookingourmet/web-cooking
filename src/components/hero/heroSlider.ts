    ...
    <article class="cg-hero-slide cg-hero-slide--program ${index === 0 ? "is-active" : ""}" data-hero-slide aria-hidden="${index !== 0}" data-program-id="${program.slug}">
      ...
      <div class="container cg-hero-slide__content">
        <div class="cg-hero-copy">
          ...
          ${heading}
          <p>${program.duration} · ${program.modality}. Formación práctica para convertir tu talento en una profesión.</p>
  ...
  return `
    <section class="cg-hero" aria-label="Programas y talleres de Cooking Gourmet">
        ...
        <article class="cg-hero-slide cg-hero-slide--workshops" data-hero-slide aria-hidden="true">
          <div class="container cg-workshop-hero">
            <div class="cg-workshop-hero__copy">
              ...
              <a class="cg-hero-btn cg-hero-btn--primary" href="${whatsapp("Hola, quiero información sobre los nuevos talleres de Cocina Peruana, Pastelería Comercial y Petit Four.")}" target="_blank" rel="noopener noreferrer" data-cta-location="hero_workshops">Ver talleres</a>
            </div>
            <div class="cg-workshop-covers" aria-label="Talleres disponibles">
              <a href="${whatsapp("Hola, quiero información sobre el taller de Cocina Peruana.")}" target="_blank" rel="noopener noreferrer" class="cg-workshop-cover"><img src="/images/portada/talleres/cocina-peruana.jpg" alt="Taller de Cocina Peruana" width="900" height="1200" loading="lazy" /><span>Cocina Peruana</span></a>
              <a href="${whatsapp("Hola, quiero información sobre el taller de Pastelería Comercial.")}" target="_blank" rel="noopener noreferrer" class="cg-workshop-cover"><img src="/images/portada/nuevos-talleres/pasteleria-comercial.jpeg" alt="Taller de Pastelería Comercial" width="1182" height="1600" loading="lazy" /><span>Pastelería Comercial</span></a>
              <a href="${whatsapp("Hola, quiero información sobre el taller Petit Four.")}" target="_blank" rel="noopener noreferrer" class="cg-workshop-cover"><img src="/images/portada/nuevos-talleres/petit-four.jpeg" alt="Taller Petit Four" width="1182" height="1600" loading="lazy" /><span>Petit Four</span></a>
            </div>
          </div>
        </article>
      </div>
      <div class="container cg-hero-nav" aria-label="Navegación del carrusel">
        ...
        <div class="cg-hero-arrows"><button type="button" data-hero-prev aria-label="Anterior">←</button><span data-hero-count>01 / 0${workshopIndex + 1}</span><button type="button" data-hero-next aria-label="Siguiente">→</button></div>
      </div>
    </section>`;
}
...
export function initHeroSlider() {
  ...
  const restart = () => { window.clearInterval(timer); if (!window.matchMedia("(prefers-reduced-motion: reduce)").matches) timer = window.setInterval(() => show(index + 1), 6500); };
